...
def train_vae(
    ...
    if model_checkpoint_path.exists():
        ...
        discriminator.load_state_dict(checkpoint_states["discriminator_state_dict"])
        ...
        if vae_lr_scheduler is not None and checkpoint_states.get("vae_lr_scheduler_state_dict") is not None:
            vae_lr_scheduler.load_state_dict(
                checkpoint_states["vae_lr_scheduler_state_dict"]
            )
        if disc_lr_scheduler is not None and checkpoint_states.get("disc_lr_scheduler_state_dict") is not None:
    ...
    with alive_bar(epochs - checkpoint_epoch, bar="fish") as bar:
        for epoch in range(checkpoint_epoch, epochs):
            vae_loss, disc_loss, global_step = train_step(vae = vae, discriminator = discriminator, disc_opt = disc_opt, vae_opt = vae_opt, loss_fn = vae_loss, train_dataloader = train_dataloader, kl_scheduler=kl_annealing_scheduler, step = global_step, device = device)
            ...
            test_vae_loss, test_disc_loss = test_step(vae = vae, discriminator = discriminator, test_dataloader = test_dataloader, loss_fn = vae_loss, kl_scheduler = kl_annealing_scheduler, step = global_step, device = device)

            print(f"epoch         : {epoch} ------------------------------------------\n",
                  f"train vae_loss: {vae_loss}     | train disc_loss : {disc_loss}\n",  
                  f"------------------------------------------------------------------\n",
                  f"test vae_loss : {test_vae_loss} | test disc_loss : {test_disc_loss}"
                )
            
            results["vae_loss"].append(vae_loss)
            ...
            if epoch >= 10:
                ...
                if min(results["test_vae_loss"]) == test_vae_loss:
                    torch.save(checkpoint_states, f"{save_model_path}/{model_name}_epoch_{epoch}_vae_test_loss_{test_vae_loss:.2f}.pt")
