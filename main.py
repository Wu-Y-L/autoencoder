...
vae_dataset = vae_dataset_loader.load_vae_dataset(input_dir = DATA_PATH, transform = transform, n_jobs = NUM_WORKERS, n_patches = N_PATCHES, patch_size = PATCH_SIZE, cache_size = CACHE_SIZE)
...
vae = autoencoder.VAE(channel_list= CHANNEL_LIST, latent_dim = LATENT_DIMS, attn_flag= ATTN_FLAGS, silu = True, int_res = True).to(device)
discriminator = loss.snPATCH_discriminator(in_channel = 1 , base_ch = 64, n_layers = 3).to(device)
...
result = train.train_vae(vae = vae, discriminator = discriminator,
                        vae_opt = vae_opt, disc_opt = disc_opt,
                        vae_loss = loss_fn, train_dataloader = train_dataloader,
                        test_dataloader = test_dataloader,epochs = EPOCHS, vae_lr_scheduler= None,
                        disc_lr_scheduler= None, model_name = "VAE_run_1", 
